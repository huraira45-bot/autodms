...
**Service Advisor Tablet App — Phases 0–1 (2026-09-14).** Plan: `C:\Users\ServerDeskop\.claude\plans\do-you-have-database-glowing-crayon.md`. Android (Capacitor 8) shell in `Software/mobile-android` around the frontend's `/tablet/*` routes; backend `/api/service-intake/*` behind `workshop_tablet`.
- ✅ Phase 0: server address + sign-in, Tablet tests (ping, video upload speed, print in-app vs Chrome), public `GET /api/service-intake/ping`, migration 133.
- ✅ Phase 1 (code): migration 135 (`dms_ServiceEstimates`, `dms_ServiceEstimateLines`, `dms_ServiceMedia`, `seq_ServiceEstimateNo` → EST-00001). Estimate is the pre-authorisation document — no job card until the customer signs. Server resolves vehicle, descriptions, prices and PST (labour) / GST (parts); labour qty forced to 1; tablet sends only ids + part qty. Walk-around media in `uploads/service-media` (200 MB cap), NOT publicly served (carved out of the `/uploads` static mount in server.js). Tablet-scoped customer search/add and vehicle add (insert-only wrappers around workshopController). Screens: home tiles, Estimates list, 4-step editor with autosave, A4 estimate print.
- ⏳ Phase 0 results from the real tablet not yet reported — in-app print route undecided.
- ✅ Phase 2 (5adc9be): signature opens the job card in one transaction (migration 136). Content hash of what the customer saw is re-checked under a row lock; tax-rate change since save refused. Job number entered by the advisor as on the desk form. Additional work = new estimate revision appended to the same JC. saveJobCard create branch + labour lines → services/jobCardSaveService.js; issuePartsToJobCard body → services/partsIssueService.js (before/after capture of the desk endpoints identical). Desk re-save now carries BayNo / TechnicianId / PerformedByName / JobStartTime / JobEndTime to the matching labour line (previously erased on every save).
- ✅ Phase 3 (5733065): desk Parts Counter `/parts-requisitions` (`parts_requisition`) issues through partsIssueService; issued qty derived live from linked Parts Issue lines. Bay screens `/tablet/bay` (`workshop_bay_screen`, dms_BayScreenDevices, migration 137): scoped device token (`scope: 'bay_screen'`) works only on `/api/bay-screen` (mounted before auth; middleware/auth.js refuses scoped tokens); Start/Finish/Undo(5 min) on server clock, auto WorkshopStatus Being Serviced → Final Inspection. socket.io namespace `/service` (services/serviceEvents.js), ids-only events + 15 s polling.
- ✅ Phase 4: tablet job cards `/tablet/job-cards` (controllers/serviceJobCardsController.js): progress, additional work, finalize wrapper (blocks on parts waiting / unsigned additional work, then delegates to finalizeController with its CNIC/DOB and 428 DMS rules), Work Order print via `WorkOrderPrint apiBase`. Only JCs opened from a tablet estimate are reachable through `/api/service-intake/job-cards`.
- ⚠ Owner: the APK must NOT be built on the DealerDesk server. Toolchain is parked on `F:\android-build`.
