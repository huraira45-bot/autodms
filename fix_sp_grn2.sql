...
-- Backfill PartyID for any rows already saved without it
UPDATE data_PurchaseInfo SET PartyID = PurchasedParty WHERE PartyID IS NULL AND PurchasedParty IS NOT NULL;
PRINT CONCAT('Backfilled rows: ', @@ROWCOUNT);
