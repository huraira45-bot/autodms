...
// The payment-void approval loop writes to the same timeline.
exports.logBookingTransition = logTransition;
...
exports.getBooking = async (req, res) => {
    try {
        ...
        const payments = await pool.request().input('id', sql.Int, id)
            .query(`SELECT p.*, fv.Status AS VoucherStatus FROM dms_SalesPayments p LEFT JOIN data_FinanceVoucherInfo fv ON fv.VoucherID = p.VoucherID WHERE p.BookingID=@id ORDER BY p.ReceivedAt DESC`);
